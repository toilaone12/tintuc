                ...
                <div class="content-wrapper">
                    <div class="row">
                        <div class="col-lg-12 m-auto grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    <a href="?act=thembaiviet" class="btn btn-success px-3 py-2 mb-3">Thêm bài viết</a>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <h4 class="card-title text-uppercase">Danh sách bài viết</h4>
                                        <div class="d-flex">
                                            <form action="?act=timkiembaiviet" method="post">
                                    ...
                                    <!-- Xử lý phần danh sách bài viết -->
                                    <?php
                                    $page = isset($_GET['page']) && $_GET['page'] ? (intval($_GET['page']) * 5) - 5  : 0;
                                    $sqlCountPage = "SELECT * FROM tintuc WHERE tieu_de_tin_tuc LIKE '%" . mb_strtolower($_POST['keyword'],'UTF-8') . "%' ORDER BY id_tin_tuc DESC";
                                    $rsCountPage = mysqli_query($conn,$sqlCountPage);
                                    $countPage = ceil($rsCountPage->num_rows / 5); //lay ra 5 tin tuc moi trang
                                    $sql = "SELECT * FROM tintuc WHERE tieu_de_tin_tuc LIKE '%" . mb_strtolower($_POST['keyword'],'UTF-8') . "%' LIMIT $page,5"; // tim kiem theo tieu de
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    ...
                                                    // tu khoa ket qua se la |a|b|, bước 1: sẽ tìm kiếm những ký tự | thành bằng , kq là ,a,b, . Bước 2: sẽ dung trim để xóa dấu "," đầu với cuối chuỗi 
                                                    $the_tu_khoa = trim(str_replace('|',',',$row['the_tu_khoa']),',');
                                                ?>
                                                    <tr>
                                                        ...
                                                        <td><?= $the_tu_khoa ?></td>
                                                        <td>
                                                            <a href="?act=suabaiviet&id=<?= $row['id_tin_tuc'] ?>" class="badge badge-success text-decoration-none me-3">Sửa</a>
                                                            <a href="?act=xoabaiviet&id=<?= $row['id_tin_tuc'] ?>" class="badge badge-danger text-decoration-none">Xóa</a>
                                                        </td>
                                                    </tr>
                                                <?php
                                                }
                                    ...
                                    <nav aria-label="..." class="d-flex justify-content-center mt-5">
                                        <ul class="pagination pagination-sm">
                                            ...
                                            ?>
                                            <li class="page-item"><a class="page-link <?= isset($_GET['page']) && $i == $_GET['page']  ? 'disabled' : ''?>" href="?act=danhsachbaiviet&page=<?=$i?>" tabindex="-1"><?=$i?></a></li>
                                            <?php
                                                }
                                            ?>
                                        </ul>
